...
import { Editor, Element, NodeEntry, Node, Range, Text, Transforms, Path, RangeRef } from "slate"
...
const Children = (props: Parameters<typeof useChildren>[0]) => <React.Fragment>{useChildren(props)}</React.Fragment>
...
export const Editable = (props: EditableProps) => {
  ...
  const scheduleOnDOMSelectionChange = useMemo(() => debounce(onDOMSelectionChange, 0), [onDOMSelectionChange])
      ...
      if (!readOnly && hasEditableTarget(editor, event.target) && !isDOMEventHandled(event, propsOnDOMBeforeInput)) {
        ...
        if (
          ...
          if (anchorNode && ReactEditor.hasDOMNode(editor, anchorNode)) {
            ...
            const lastText = document.createTreeWalker(anchorNode, NodeFilter.SHOW_TEXT).lastChild() as DOMText | null
...
export const DefaultPlaceholder = ({ attributes, children }: RenderPlaceholderProps) => (
...
export const isTargetInsideNonReadonlyVoid = (editor: ReactEditor, target: EventTarget | null): boolean => {
