...
export function gatherMutationData(editor: Editor, mutations: MutationRecord[]): MutationData {
  ...
  mutations.forEach((mutation) => {
    switch (mutation.type) {
      ...
      case "characterData": {
        ...
        if (insertedText.some(({ path }) => Path.equals(path, textInsertion.path))) {
          return
        }
...
export const isLineBreak: MutationDetection = (editor, { addedNodes }) => {
  ...
  const parentNode = selection ? Node.parent(editor, selection.anchor.path) : null
  const parentDOMNode = parentNode ? ReactEditor.toDOMNode(editor, parentNode) : null
  ...
  return addedNodes.some(
    (addedNode) => addedNode instanceof HTMLElement && addedNode.tagName === parentDOMNode?.tagName
  )
}
 ...
 */
export const isReplaceExpandedSelection: MutationDetection = ({ selection }, { removedNodes }) => {
 ...
 */
export const isRemoveLeafNodes: MutationDetection = (_, { addedNodes, characterDataMutations, removedNodes }) => {
