...
function findClosestKnowSlateNode(domNode: DOMNode): SlateNode | null {
  ...
  return domEl?.parentElement ? findClosestKnowSlateNode(domEl.parentElement) : null
}

export function useRestoreDom(node: React.RefObject<HTMLElement>, receivedUserInput: React.RefObject<boolean>) {
  ...
  const restore = useCallback(() => {
    ...
    const nodesToRestore = mutated.filter(
      (n) => !mutated.some((m) => Path.isParent(ReactEditor.findPath(editor, m), ReactEditor.findPath(editor, n)))
    )
