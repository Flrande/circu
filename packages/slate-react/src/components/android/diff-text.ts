...
export function getTextInsertion<T extends Editor>(editor: T, domNode: DOMNode): TextInsertion | undefined {
...
export function normalizeTextInsertionRange(editor: Editor, range: Range | null, { path, text }: TextInsertion) {
  ...
  const isSingleCharacterInsertion = insertText.length === 1 || removeText.length === 1
  ...
  if (isSingleCharacterInsertion && Path.equals(range.anchor.path, path)) {
    const [text] = Array.from(Editor.nodes(editor, { at: range, match: Text.isText }))
