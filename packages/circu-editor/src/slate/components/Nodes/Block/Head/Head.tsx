...
const Head: React.FC<CustomRenderElementProps<IHead>> = ({ attributes, children, element }) => {
  switch (element.headGrade) {
    case "1":
      return (
        <div {...attributes} className={head_1}>
          {children}
        </div>
      )
    case "2":
      return (
        <div {...attributes} className={head_2}>
          {children}
        </div>
      )
    case "3":
      return (
        <div {...attributes} className={head_3}>
          {children}
        </div>
      )
    case "4":
      return (
        <div {...attributes} className={head_4}>
          {children}
        </div>
      )
    case "5":
      return (
        <div {...attributes} className={head_5}>
          {children}
        </div>
      )
    case "6":
      return (
        <div {...attributes} className={head_6}>
          {children}
        </div>
      )
  }
